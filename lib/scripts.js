...
var listaNotas = {
    ...
    section: undefined.document.getElementsbyClassName("Notes")[0],
...
function updateSection(section) {
    ...
    for (var index = 0; index < listaNotas.listaInterna.length; index++) {
        if (listaNotas.listaInterna[index].editing) {
            innerHTML += '<form class="note">' + '<input class="note__title" type="text" name="title" value="' + listaNotas.listaInterna[index].title + '" placeholder="Título" />' + '<textarea class="note__body" name="body" rows="5" placeholder="Criar uma nota...">' + listaNotas.listaInterna[index].body + '</textarea>' + '<button class="note__control" type="button" onclick="onDoneClick(this.form, this.form.parentElement, this.form.title, this.form.body, ' + index + ')">' + 'Concluído' + '</button>' + '</form>';
        } else {
            innerHTML += '<form class="note" onclick="onEditClick(this.parentElement, ' + index + ')">' + '<button class="note__control" type="button" onclick="onRemoveClick(this.form.parentElement, ' + index + ')">' + '<i class="fa fa-times" aria-hidden="true"></i>' + '</button>' + '<h1 class="note__title">' + listaNotas.listaInterna[index].title + '</h1>' + '<p class="note__body">' + listaNotas.listaInterna[index].body + '</p>' + '</form>';
        }
    }
